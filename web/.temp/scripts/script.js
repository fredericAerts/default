...
if(!noGlobal){window.jQuery = window.$ = jQuery;}return jQuery;});var defaultApp=defaultApp || {};defaultApp.nav = (function($,window,undefined){var init,testFunction;init = function(){};testFunction = function(test){return function(){};};return {init:init};})(jQuery,window);var defaultApp=defaultApp || {};defaultApp.app = (function($,window,undefined){var init=undefined,initOnload=undefined,appScroll=undefined,debounce=undefined; // On Dom ready
init = function(){defaultApp.nav.init();console.log('hello','init');}; // On All loaded
initOnload = function(){appScroll();}; // On Scroll
appScroll = function(){var _onScroll=undefined;_onScroll = debounce(function(){console.log('scrolling');},50);$(window).scroll(function(){_onScroll();});};debounce = function(func,wait,immediate){var timeout=undefined;return function(){for(var _len=arguments.length,args=Array(_len),_key=0;_key < _len;_key++) {args[_key] = arguments[_key];}var context=undefined;var later=function later(){timeout = null;if(!immediate){func.apply(context,args);}};var callNow=immediate && !timeout;clearTimeout(timeout);timeout = setTimeout(later,wait);if(callNow){func.apply(context,args);}};};return {init:init,initOnload:initOnload};})(jQuery,window); // Dom ready
$(function(){defaultApp.app.init();}); // All loaded
$(window).load(function(){defaultApp.app.initOnload();}); // Otherwise append directly
